 ...
 * @brief Call VmDatabase's Submit request and return a unique identifier
    ...
    )
{
    ...
    try
    {
        VmDatabase * poVmDatabase = ::GetVmDatabase();
        __DebugAssert(nullptr != poVmDatabase);
        if ((nullptr != c_pbSerializedRequest)&&(0 < unSerializedRequestSizeInBytes)&&(nullptr != punSerializedResponseSizeInBytes))
        {
            ...
            un64Identifier = poVmDatabase->SubmitRequest(oRequest, punSerializedResponseSizeInBytes);
        }
 ...
 * @brief Call VmDatabase's GetResponse and return response associated
    ...
    )
{
    ...
    try
    {
        ...
        {
            fSuccess = poVmDatabase->GetResponse(un64Identifier, pbSerializedResponseBuffer, unSerializedResponseBufferSizeInBytes);
        }
    }
 ...
 *        function. This function creates  singleton object of VmDatabase class and
 *        calls VmDatabase's InitializePlugin, which initializes the plugin's
 *        data members including its dictionary. This function then calls RegisterPluginFn which is used to
 *        register VmDatabase's dictionary with the RestFramework
    ...
    )
{
    ...
    try
    {
        VmDatabase * poVmDatabase = ::GetVmDatabase();
        __DebugAssert(nullptr != poVmDatabase);
        poVmDatabase->InitializePlugin();
        std::vector<Byte> stlDictionary = poVmDatabase->GetDictionarySerializedBuffer();
        fSuccess = fnRegisterPluginFn(poVmDatabase->GetName(), poVmDatabase->GetUuid(), poVmDatabase->GetVersion(), SubmitRequest, GetResponse, stlDictionary.data(), stlDictionary.size());
    }
 ...
 * @brief Call VmDatabase to release the object resources and
