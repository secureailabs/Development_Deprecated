    ...
    )
{
    ...
    for (auto const & element : c_oStructuredBuffer.m_stlMapOfElements)
    {
        StructuredBufferElement * poCopy = new StructuredBufferElement(*(element.second));
        m_stlMapOfElements[element.first] = poCopy;
    }
    m_stlSerializedBuffer = c_oStructuredBuffer.m_stlSerializedBuffer;
    m_qw64BitHash = c_oStructuredBuffer.m_qw64BitHash;
    m_qwComposition64BitHash = c_oStructuredBuffer.m_qwComposition64BitHash;
}
...
const Byte * __thiscall StructuredBuffer::GetSerializedBufferRawDataPtr(void) const throw()
{
    ...
    __DebugAssert(0 != m_qw64BitHash);
    __DebugAssert(0 != m_qwComposition64BitHash);
...
std::vector<std::string> __thiscall StructuredBuffer::GetDescriptionOfElements(void) const throw()
{
    ...
    for (auto const & element : m_stlMapOfElements)
    {
        stlListOfElements.push_back("Name[" + element.second->GetElementName() + "],Type[" + gs_aszTypeNames[element.second->GetElementType()] + "],Size[" + std::to_string(element.second->GetRawDataSizeInBytes()) + " Bytes]");
    }
...
/********************************************************************************************/
//
// Big number are complex types returned within an StructuredBuffer

StructuredBuffer __thiscall StructuredBuffer::GetBigNumber(
    _in const char * c_szElementName
    ) const
{
    __DebugFunction();

    const StructuredBufferElement * c_poStructuredBufferElement = m_stlMapOfElements.at(this->CheckForElement(c_szElementName, BIG_NUMBER_TYPE));
...
void __thiscall StructuredBuffer::PutBigNumber(
    _in const char * c_szElementName,
    _in const Byte * c_pbByteArray,
    _in bool fIsNegative,
    _in unsigned unSizeInBits
    )
{
    __DebugFunction();
    
    StructuredBuffer oBigNumber;
    unsigned int unSizeInBytes = (0 == (unSizeInBits % 8)) ? (unSizeInBits / 8) : ((unSizeInBits / 8) + 1);
    
    oBigNumber.PutBuffer("Data", c_pbByteArray, unSizeInBytes);
    oBigNumber.PutUnsignedInt32("NumberOfBits", (uint32_t) unSizeInBits);
    oBigNumber.PutBoolean("IsNegative", fIsNegative);
    
    this->AddElement(c_szElementName, BIG_NUMBER_TYPE, (const void *) oBigNumber.GetSerializedBufferRawDataPtr(), oBigNumber.GetSerializedBufferRawDataSizeInBytes());
}
